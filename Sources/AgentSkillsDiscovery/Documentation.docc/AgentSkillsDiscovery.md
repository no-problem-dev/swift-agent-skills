...
バリデーションエラーがあっても読み込み可能な限り**寛容に（warn-and-load）**ロードします。
OpenHands の探索仕様と公式クライアントガイドを移植した設計です。

ファイルシステム依存は `FileSystemReading` / `FileSystemWriting` プロトコル
（`swift-persistence` の `PersistenceCore` ）に集約されているため、
本番環境では実 OS ファイルシステムを、テスト環境ではインメモリ実装を差し込んで使えます。

### スキルの探索と読み込み

`SkillDiscoveryConfig` で探索ルートを設定し、`FileSystemSkillDiscovery` を使って `discover()` を呼ぶと
`DiscoveredSkills` が返ります。複数ルートで同名スキルがあった場合は `SkillScope`
（`explicit > project > user`）で上位のものが優先され、衝突は `SkillDiagnostic` として記録されます。
...
`SkillRegistry` は探索結果をキャッシュし、`SkillPolicy` によるフィルタリングを提供します。
ビルトインスキルをシードしてから、ディスク上のスキルで上書きするパターン
（ユーザーがビルトインを同名スキルで差し替えられる）を実装しています。
...
`SkillWriter` は書き込み操作を担います。`SkillDocument.serialize` で生成した内容を
インジェクトされた `FileSystemWriting` バックエンドへ永続化します。
