...
`AgentSkillsTool` はパッケージの中で唯一 `swift-llm-client` に依存するモジュール。
エージェントループに登録する `invoke_skill` ツール（`InvokeSkillTool`）を 1 つ提供する。

このツールは構造上、Tier-1 カタログ（`<available_skills>` ブロック）と
Tier-2 有効化（`name` パラメータの列挙）を**同一のスキルスナップショットから生成**する。
モデルがカタログで見えるスキルと、ツールで呼び出せるスキルが必ず一致する。

カタログは `Tool.systemInstruction` に乗ってループが自動的にシステムプロンプトへ注入するため、
呼び出し側は `InvokeSkillTool.make(skills:activator:)` の戻り値をツールリストに追加するだけ。
