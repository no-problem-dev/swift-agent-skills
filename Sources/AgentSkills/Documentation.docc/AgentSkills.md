...
`AgentSkills` は [Agent Skills 標準](https://agentskills.io) の Swift 移植です。
`SKILL.md` フロントマターのパース・バリデーション・シリアライズ、
そしてエージェントのシステムプロンプトへ注入する `<available_skills>` カタログ生成を担います。

このモジュールは一切の副作用を持たない**ピュア層**です。
ファイルシステムへの直接アクセスは行わず、`FileSystemReading` プロトコルを介したインジェクション形式を取ります。
そのため単体テストでインメモリ実装を差し込むことができ、CLI・サーバーの両環境で安全に利用できます。

### パッケージの構成

`swift-agent-skills` パッケージは 4 つのライブラリから成ります。
このモジュール（`AgentSkills`）が土台で、残り 3 つがそれぞれの役割を担います。

ファイルシステムからスキルをスキャンして読み込む探索層は **`AgentSkillsDiscovery`** が担います。
`FileSystemSkillDiscovery` によるマルチルート探索、`SkillRegistry` によるスキルの保持と検索、
`SkillWriter` によるスキル作成・更新・削除が含まれます。

スキルを有効化してエージェントループへ注入する実行層は **`AgentSkillsRuntime`** が担います。
`SkillActivator` による名前解決と重複排除、`SkillCatalogRenderer` による `<available_skills>` ブロックのレンダリング、
`SkillSessionState` によるセッション内アクティベーション履歴の管理が含まれます。
このモジュールも LLM スタックへの依存を持たないピュア層です。

LLM ツールとしての統合層は **`AgentSkillsTool`** が担います。
`InvokeSkillTool` 1 つだけが `swift-llm-client` の `Tool` プロトコルに準拠し、
カタログと `name` 列挙を単一のスナップショットから生成します。
